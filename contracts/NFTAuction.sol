...
contract NFTAuction is Ownable, ReentrancyGuard {
    ...
    constructor(address _nftContract, address payable _charityWallet) Ownable(msg.sender) {
        nftContract = BasicNFT(_nftContract);
        charityWallet = _charityWallet;
    }

    function listNFT(uint256 tokenId, uint256 biddingTime) public {
        ...
        delete auction.bidders;  // Clear previous bidders list
    ...
    function auctionEnd(uint256 tokenId) public nonReentrant {
        Auction storage auction = auctions[tokenId];
        ...
        // Transfer highest bid to charity wallet
        charityWallet.transfer(auction.highestBid);
        ...
        for (uint256 i = 0; i < auction.bidders.length; i++) {
            ...
            if (bidder != auction.highestBidder) {
                uint256 amount = auction.pendingReturns[bidder];
                if (amount > 0) {
